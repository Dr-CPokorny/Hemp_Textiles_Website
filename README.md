...
(`CTRL+SHIFT++` for open all and `CTRL+SHIFT+-` for close all).
...
I am not using Node.js. Simply put, screw Node.js; it's overly complicated. It's outdated and I hate it for how slow and bloated it is.
...
#### Build Tree for Core Builders

![A tree showing that core builders send information via a bubble up method. build_page is the root. The 1st child level has build_html. The 2nd child level has build_head and body_director. The 3rd child level has 4 different body builders (one for each page.)](docs/assets/images/build_tree.png)

Every core builder besides `build_page` (the root) sends information up to help construct a page. This allows for pages to be constructed
without much repetition. There is no need to make a different head or html tag for every page since their respective builders handle that.
The idea of this framework is to build down the tree and then send information back up to create `.html` pages with little repetition
in the coding.

#### Core Builders

These are the builders used to write files, create the outer frame for an html page, develop the head logic
for an html page, to create direction for which body to construct for a page, and to ultimately
create the unique content within each page. Here is the list for core builders:
...
- `body_director`
- `build_body_for_home`
- `build_body_for_learn`
- `build_body_for_about`
- `build_body_for_sources`

#### Content Builders

The builders for the body tags have repetitive content within them. Some content can even be shared across pages (navigation and footer), others
are repetitive blocks that might be used to fulfill the purpose of a page (references for the Sources page). This repetitive content is managed
through content builders so as to avoid duplicating code. Here are all the current content builders:

- `build_navigation`
- `build_footer`
- `build_top` — Creates the top of each page. Title and subtext.
- `build_person_for_about_page`
- `build_notice_blurb_for_home_page` — The content directly below the survey buttons on the home page.
- `build_card_for_home_page`
- `build_why_card_for_learn_page`
- `build_misconception_card_for_learn_page`
- `build_material_test_for_learn_page`
- `build_left_row_for_comparison_chart_for_learn_page`
- `build_right_row_for_comparison_chart_for_learn_page`
...
Through 3 parts using a project I've been working on. You can find the project details here:
https://github.com/Mirth-Development/style_management

Here are the 3 sequential parts:

1. Make a style definition.
2. The style manager processing a definition.
3. Load the style definition into the pages through a `DOMContentLoaded` event.

#### Style Manager

Used to process a style definition and help apply its declarations by making use of the `Style` type that the manager can help
create. The `Style` type has a multitude of methods that allow different ways of applying a style definition, whether it be all at once
or by selective parts of a definition during specified event handlers. Realistically speaking, the `style_manager.ts` file should **never**
be touched unless you want to change how style definitions are managed and applied.

The Style Manager does not support CSS events such as `:hover`. Rather, all events are to be handled via TypeScript/JavaScript events.
This ensures events don't take place in both CSS and script files, rather they're all in script events since the style manager forces
such a thing (don't mix and match purpose please).

#### Style Definition

A collection of objects that hold style declarations; `STYLE_HEAVENLY_EARTH` is an example. You can specify different types of selectors using the prefixes `u`,
`i`, `c`, and `t` (`o` is for a parent object which should selector prefix objects). Think of a style definition as a tree, where
each `o` prefixed object adds to a selector name (ensures all selectors have unique names).
...
of sources when an id-jump (hash jump) occurs was 70% generated; I made sure to check everything, I hate blindly accepting code.

Ultimately, around **5% to 10%** was truly vibe coded, and I made sure to go through any vibe crap thoroughly before it
got put into the end product.
...
> 🖼️ **PNG Compressor:** https://tinypng.com/
...
**I AM _NOT_ A PROGRAMMER:**
Go into the HTML files and change the contents there. Afterwards, push your changes to GitHub using either git commands or a git interface such as GitHub Desktop.

**I AM A PROGRAMMER:**
Manipulate the builders and regenerate the HTML using a Bun command. Afterwards, push your changes to GitHub using either git commands or a git GUI such as GitHub Desktop.
