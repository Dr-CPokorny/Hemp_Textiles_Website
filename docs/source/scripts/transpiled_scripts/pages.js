...
function build_body_for_home() {
    // Top Information
    const TOP_INFO = build_top("\t\t", "Help Shape the Future of Hemp in the Textile Arts", "Share your experience or curiosity about hemp fabrics.<br>Your input informs sustainable textile innovation.");
    ...
    const CARD_SUSTAINABLE = build_card_for_home_page("\t\t\t\t", "assets/images/home_card_sustainable.svg", "Finished patterned quilt made with hemp fabric.", "Sustainable", "Hemp is a low-impact crop that supports a healthier planet.");
    const CARD_DURABLE = build_card_for_home_page("\t\t\t\t", "assets/images/home_card_durable.svg", "Finished white quilt made with hemp fabric.", "Durable & Beautiful", "Strong fibers, soft textures, made to last in every stitch.");
    const CARD_MAKERS = build_card_for_home_page("\t\t\t\t", "assets/images/home_card_makers.svg", "Finished blue quilt made with hemp fabric.", "Made for Makers", "Crafters like you are leading the way to a greener future.");
    return [
...
function build_body_for_learn() {
    // Top Information
    const TOP_INFO = build_top("\t\t", "Getting To Know Hemp", "Hemp has a long history as a natural fiber, but certain myths have kept it out of textile markets.  This page is dedicated to abolishing those myths.");
    ...
    const MARIJUANA_THC = build_right_row_for_comparison_chart_for_learn_page("\t\t\t\t\t", "arrow_warm_up", "High THC (>0.3%)");
    ...
    // Why Card Information
    const WHY_CARD_DURABLE = build_why_card_for_learn_page("\t\t\t\t", "assets/images/learn_why_card_durable.svg", "Coat made of hemp textiles used to show the durability factor behind hemp.", "Durable and Long-Lasting", `Hemp fibers are known for their strength and long-lasting quality ${build_inline_source("R-005", "R-008")}.`);
    const WHY_CARD_BREATHABLE = build_why_card_for_learn_page("\t\t\t\t", "assets/images/learn_why_card_breathable.svg", "Large white hemp quilt with star patterns weaved in.  Even though the quilt is large, it looks to be both light and soft.", "Breathable and Lightweight", `Hemp fabric is breathable and gets softer with every wash and use ${build_inline_source("R-009", "R-025")}; there is a limit to how soft it can get.`);
    const WHY_CARD_TEXTURED = build_why_card_for_learn_page("\t\t\t\t", "assets/images/learn_why_card_textured.svg", "Pieces of hemp fabric are displayed in different colors and put next to flowers and fungus to showcase its textural and color likeness to natural elements in the surrounding environment.", "Naturally Textured and Tactile", `Its natural texture adds depth and character to textile work ${build_inline_source("R-008", "R-009")}.`);
    const WHY_CARD_BIODEGRADABLE = build_why_card_for_learn_page("\t\t\t\t", "assets/images/learn_why_card_biodegradable.svg", "A bag made from hemp textiles that has been placed on a rock within a forest.  Possibly present to showcase that the bag could be left and biodegradation would take place?", "Biodegradable and Lower-Impact", `Hemp requires less water and fewer pesticides than cotton ${build_inline_source("R-001", "R-002", "R-006", "R-010")}.`);
    // Misconception Card Information
    const MISCONCEPTION_CARD_COMPARISON = build_misconception_card_for_learn_page("\t\t\t\t", '"Hemp is the same as marijuana."', `While hemp and marijuana come from the same plant species ${build_inline_source("R-027")}, they are grown and used very differently. Textile hemp contains very low THC levels ${build_inline_source("R-027")} and is used for industrial applications like fabric, rope, and paper ${build_inline_source("R-007", "R-012")}.`);
    const MISCONCEPTION_CARD_ROUGH = build_misconception_card_for_learn_page("\t\t\t\t", '"Hemp fabric is rough."', `Modern hemp textiles can range from structured and textured to soft and breathable depending on the weave and blend ${build_inline_source("R-025")}. Many hemp fabrics become softer over time with washing and regular use ${build_inline_source("R-009")}.`);
    const MISCONCEPTION_CARD_TEXTILE_ARTS = build_misconception_card_for_learn_page("\t\t\t\t", '"Hemp isn\'t practical for the textile arts."', "Part of this project explores exactly that question. By gathering feedback from quilters and testing material experiences, the project aims to better understand hemp's usability, comfort, and creative potential for handmade craftsmanship.");
    // Material Tests Information
    const TEST_TEAR = build_material_test_for_learn_page("\t\t\t\t", "assets/images/learn_test_tear.svg", "A bar chart showing the differences between warp and weft tears of 4 different fabrics, the chart shows how much force it takes to cause a tear in a given direction.", "Tear Test Results", "The tear test showed that the textured woven cotton had the highest tear strength in the warp direction, while hemp and cotton/linen also performed well with consistent results. Hemp fabric showed balanced tear resistance between warp and weft directions, making it a strong and reliable option compared to basic quilt cotton. The textured woven cotton had a large difference between warp and weft strength, showing that fabric direction can greatly affect performance.");
    const TEST_ABRASION = build_material_test_for_learn_page("\t\t\t\t", "assets/images/learn_test_abrasion.svg", "A bar chart showing how many abrasion revolutions it took for 4 different fabrics to wear out.  The hemp/cotton blend took the longest to wear out.", "Abrasion Test Results", "The abrasion test showed that hemp fabric had the highest resistance to wear, lasting an average of about 45 revolutions before failure. Cotton/linen and textured woven cotton performed in the middle range, while basic quilt cotton showed the lowest abrasion resistance. Overall, the hemp fabric handled repeated wear the best, which supports its potential as a durable material for quiltmaking and other textile uses.");
    const TEST_DRAPE = build_material_test_for_learn_page("\t\t\t\t", "assets/images/learn_test_drape.svg", "A bar chart showcasing 4 different fabrics and their average drape coefficients.  The hemp/cotton blend had the second highest, while the cotton fabric had the highest drape coefficient average.", "Drape Test Results", "The drape test was used to evaluate how naturally each fabric hangs and conforms under its own weight. A higher drape coefficient indicates a stiffer fabric, while a lower coefficient indicates greater flexibility and flow. Results showed that the hemp/cotton blend performed similarly to traditional cotton, suggesting that hemp-based textiles can provide comparable handling characteristics for textile-crafting applications. Linen/cotton and woven cotton fabrics exhibited lower drape coefficients, indicating greater flexibility. These findings suggest that hemp blends may offer a sustainable alternative to cotton while maintaining familiar fabric behavior valued by textile artists.");
    const TEST_SHRINKAGE = build_material_test_for_learn_page("\t\t\t\t", "assets/images/learn_test_shrinkage.svg", "A bar chart showcasing 4 different fabrics and their average shrinkage in inches after being washed.  100% Cotton Kona had the least shrinkage (1st place), while the hemp/cotton blend had the 2nd highest shrinkage (3rd place).", "Shrinkage Test Results", "For the shrinkage test, each fabric sample was measured before and after washing to compare how much the material changed in size. The results showed that 100% Cotton Kona had the lowest shrinkage, with an average of about 0.203 inches, making it the most dimensionally stable fabric in this test. The hemp/cotton blend and Essex linen blend had very similar shrinkage results, averaging about 0.48 inches and 0.47 inches, which shows that the hemp blend performed close to another common natural fiber blend. The Diamond Textiles textured cotton had the highest shrinkage at about 0.625 inches. Overall, this test helped show that hemp fabric is still a usable option for quilting, but like many natural fabrics, it should likely be pre-washed before use to reduce shrinking after a quilt is made.");
    return [
        `   <body class="${SS.form_static_column_center}">`,
        ``,
        `       ${build_navigation("\t\t")}`,
        ...
        `                   <h3 class="${SS.learn_compare_column_heading_right}">MARIJUANA</h3>`,
        `                   ${MARIJUANA_THC}`,
        `                   ${MARIJUANA_USE}`,
        `                   ${MARIJUANA_PSYCHO}`,
        `               </div>`,
...
function build_material_test_for_learn_page(indent, image_path, image_alt, title, text) {
    return [
        `<!-- ${title} -->`,
